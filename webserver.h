...
public:
    int m_port;             //端口号
    char *m_root;           
    int m_log_write;        //日志写入方式，1表示异步写入日志，0表示同步写入日志
    int m_close_log;        //是否关闭日志
    int m_actormodel;       //并发模式选择
    int m_pipefd[2];
    int m_epollfd;
    http_conn *users;
    ...
    //线程池相关
    threadpool<http_conn> *m_pool;  //处理http连接的线程池
    int m_thread_num;               //线程池内线程数量

    //epoll_event相关
    epoll_event events[MAX_EVENT_NUMBER];       //最大事件数1000
    int m_listenfd;
